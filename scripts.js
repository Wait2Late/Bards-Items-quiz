...
function getChampionDisplayNameFromId(championId) {
    var _a;
    try {
        if (!allChampions || !allChampions.data)
            return null;
        // Direct key lookup first (most ids are also object keys)
        var byKey = allChampions.data[championId];
        if (byKey && byKey.name)
            return byKey.name;
        // Fallback: scan values in case keys differ
        var match = Object.values(allChampions.data).find(function (c) { return c.id === championId; });
        return (_a = match === null || match === void 0 ? void 0 : match.name) !== null && _a !== void 0 ? _a : null;
    }
    catch (_b) {
        return null;
    }
...
function getTeamChampions() {
    ...
    myTeamSlots.forEach(function (selectElement) {
        ...
        if (championId) {
            ...
            var ddName = getChampionDisplayNameFromId(championId) || ((_a = select.selectedOptions[0]) === null || _a === void 0 ? void 0 : _a.textContent) || championId;
            var normalizedName_1 = normalizeName(ddName);
            var champion = championWikiData.find(function (c) { return normalizeName(c.name) === normalizedName_1; });
            if (champion)
    ...
    enemyTeamSlots.forEach(function (selectElement) {
        ...
        if (championId) {
            var ddName = getChampionDisplayNameFromId(championId) || ((_a = select.selectedOptions[0]) === null || _a === void 0 ? void 0 : _a.textContent) || championId;
            var normalizedName_2 = normalizeName(ddName);
            var champion = championWikiData.find(function (c) { return normalizeName(c.name) === normalizedName_2; });
            if (champion)
    ...
    team.forEach(function (Champion) {
        switch (Champion.class) {
            ...
            case "Specialist":
                ...
                break;
            default:
                console.log("Against ".concat(Champion.name, " (").concat(Champion.class, "), consider balanced items."));
        }
    });
    return { ap: ap, ad: ad, tank: tank };
}
